...
export function prepBarGraph(data){
    data = sortByAdjustedPercentageLikes(data)
    var series = []
    var names = []
    for (var i = 0; i< 10; i++){
        names.push(`${i+1}: ${data[i].emoji} - ${toTitleCase(data[i].name)}`)
        series.push(((data[i].positive/data[i].count)*data[i].multiplier).toFixed(4))
    }
      ...
      options: {
        ...
        tooltip: {
          ...
          y: {
            title: {
              formatter: function (val, opt) {
                return opt.w.globals.labels[opt.dataPointIndex].substring(3,6) + '\'s Adjusted Approval Rating:'
              }
            }
          }
        }
...
export function sortByAdjustedPercentageLikes(stats){
  var tmp = []
  for (const map in stats){
      tmp.push({name:map, ...stats[map]})
  }
  ...
  function compare( a, b ) {
      if (((a.positive/a.count)*a.multiplier) < ((b.positive/b.count)*b.multiplier)){
          return 1;
      }
      if (((a.positive/a.count)*a.multiplier) > ((b.positive/b.count)*b.multiplier)){
          return -1;
